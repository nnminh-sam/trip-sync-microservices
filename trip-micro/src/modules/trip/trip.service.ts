...
export class TripService {
  ...
  async getTripStatisticOfEmployee(
    ...
    const query = this.tripRepo
      ...
      .where('trip.deletedAt IS NULL')
      .andWhere('trip.assigneeId = :employeeId', { employeeId })
      // optional but recommended: enforce manager ownership
      .andWhere('trip.managerId = :managerId', { managerId });
    ...
    for (const trip of items) {
      if (trip.evaluation_value === null) {
        unevaluated++;
        continue;
      }

      evaluated++;
      statistic[trip.evaluation_value] =
        (statistic[trip.evaluation_value] ?? 0) + 1;
    }
