...
import { GPSExport, ExportStatus, ExportFormat } from 'src/models/gps-export.model';
...
export class GpsService {
  ...
  async trackGPS(dto: TrackGPSDto, userId: string): Promise<TrackGPSResponseDto> {
    this.logger.log(`Tracking GPS for trip ${dto.tripId} at ${dto.timestamp}`);
    
    try {
      // Check for duplicate timestamp
      const existing = await this.gpsLogRepo.findOne({
        where: {
          trip_id: dto.tripId,
          user_id: userId,
      ...
      if (existing) {
        throwRpcException({
          ...
          message: 'GPS log already exists for this timestamp',
        });
      }
  ...
  async batchTrackGPS(dto: BatchTrackGPSDto, userId: string): Promise<BatchTrackGPSResponseDto> {
    this.logger.log(`Batch tracking ${dto.locations.length} GPS points for trip ${dto.tripId}`);
    
    const duplicates: string[] = [];
    const failed: Array<{ index: number; timestamp: string; error: string }> = [];
    const toSave: GPSLog[] = [];
    ...
    if (toSave.length > 0) {
      await this.gpsLogRepo.save(toSave);
    }
    
    return {
      processed: toSave.length,
      message: `${toSave.length} GPS locations logged successfully`,
  ...
  async checkIn(dto: CheckInDto, userId: string, claims: TokenClaimsDto): Promise<CheckInResponseDto> {
  ...
  async checkOut(dto: CheckOutDto, userId: string, claims: TokenClaimsDto): Promise<CheckOutResponseDto> {
    ...
    try {
      ...
      const duration = Math.round(
        (new Date(dto.timestamp).getTime() - checkIn.timestamp.getTime()) / 60000
      );
  ...
  // Route Methods
  async getTripRoute(tripId: string, query: RouteQueryDto): Promise<TripRouteResponseDto> {
  ...
  async detectStops(tripId: string, query: StopQueryDto): Promise<TripStopsResponseDto> {
    ...
    for (const log of logs) {
      ...
      const timeDiff = (log.timestamp.getTime() - lastLog.timestamp.getTime()) / 60000;
      ...
      if ((log.speed && log.speed < 5) || distance < 50) {
        ...
        const stopDuration = 
          (new Date(currentStop.departureTime).getTime() - 
           new Date(currentStop.arrivalTime).getTime()) / 60000;
  ...
  async getTripStatistics(tripId: string): Promise<TripStatisticsDto> {
    ...
    const duration = Math.round(
      (logs[logs.length - 1].timestamp.getTime() - logs[0].timestamp.getTime()) / 60000
    );
    ...
    const efficiencyScore = movingTime > 0 
  ...
  async getAnalyticsSummary(query: GPSAnalyticsQueryDto): Promise<GPSAnalyticsSummaryDto> {
    ...
    for (const [tripId, logs] of tripLogs) {
      ...
      if (tripDistance > longestTrip.distance) {
        longestTrip = { tripId, distance: tripDistance, duration: tripDuration };
      }
    }
  ...
  // Real-time Methods
  async getRealtimeLocations(query: RealtimeLocationQueryDto): Promise<RealtimeLocationsResponseDto> {
  ...
  // Export Methods
  async createExport(dto: CreateGPSExportDto, userId: string): Promise<CreateGPSExportResponseDto> {
  ...
  async getExportStatus(exportId: string): Promise<GPSExportStatusResponseDto> {
    ...
    return {
      ...
      metadata: {
        ...
        fileSize: exportJob.file_size ? `${(exportJob.file_size / 1024 / 1024).toFixed(2)}MB` : undefined,
  ...
  private simplifyRoute(route: RoutePointDto[], tolerance: number = 0.0001): RoutePointDto[] {
    ...
    for (let i = 1; i < route.length - 1; i++) {
      ...
      if (distance > 50 || // 50 meters
          (point.speed && prevPoint.speed && Math.abs(point.speed - prevPoint.speed) > 10) ||
          (point.heading && prevPoint.heading && Math.abs(point.heading - prevPoint.heading) > 30)) {
