...
export class LocationService {
  ...
  async create(claims: TokenClaimsDto, createLocationDto: CreateLocationDto) {
    ...
    try {
      ...
      this.logger.log(
        `Location created successfully: ${createLocationDto.name}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to create location: ${createLocationDto.name}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findAll(claims: TokenClaimsDto, filterDto: FilterLocationDto) {
    ...
    try {
      ...
      this.logger.log(
        `Found ${Array.isArray(result) ? result.length : 0} locations`,
      );
      return result;
    } catch (error) {
